...
[ -z "$WLS_ADMIN_USER" ] && { read -rp "  WLS admin user [weblogic]: " WLS_ADMIN_USER; }
WLS_ADMIN_USER="${WLS_ADMIN_USER:-weblogic}"
[ -z "$WLS_ADMIN_PASS" ] && { read -rsp "  WLS admin password: " WLS_ADMIN_PASS; echo ""; }
[[ -z "$WLS_ADMIN_PASS" ]] && { log_erro "Senha do WLS admin obrigatória."; exit 1; }
