...
NOVA_EXPR_COMPLETA = '|'.join(GRUPOS)

print('Conectando em ' + ADMIN_URL + '...')
connect(ADMIN_USER, ADMIN_PASS, ADMIN_URL)

CONFIG_PATH = '/SecurityConfiguration/' + DOMAIN_NAME + '/DefaultRealm/myrealm/RoleMappers/XACMLRoleMapper'

# --- Diagnostico: descobrir API disponivel no MBean ---
print('')
print('=== DIAGNOSTICO XACMLRoleMapper ===')
try:
    cd(CONFIG_PATH)
    metodos = sorted([m for m in dir(cmo) if not m.startswith('_')])
    print('Metodos disponiveis: ' + str([m for m in metodos if 'role' in m.lower() or 'express' in m.lower() or 'resource' in m.lower()]))
    print('Todos: ' + str(metodos))
except Exception as e_diag:
    print('Diagnostico falhou: ' + str(e_diag))
print('===================================')
print('')

# --- Tentativa 1: config tree, resource=None (WLS 12.2.1.4) ---
sucesso = False
try:
    cd(CONFIG_PATH)
    expr_str = ''
    try:
        ...
        print('Sem expressao previa. Criando do zero.')
    ...
    if grupos_add:
        ...
        print('Configurado via config tree (resource=None).')
        sucesso = True
    else:
        print('Todos os grupos ja presentes.')
        sucesso = True
except Exception as e1:
    print('Config tree falhou: ' + str(e1.__class__.__name__) + ' — ' + str(e1)[:120])

# --- Tentativa 2: domainRuntime() ---
if not sucesso:
    print('Tentando domainRuntime()...')
    try:
        domainRuntime()
        for rt_path in [
            '/SecurityRuntime/myrealm/RoleMappers/XACMLRoleMapper',
            '/SecurityRuntime/RoleMappers/XACMLRoleMapper',
            '/DomainRuntime/SecurityConfiguration/' + DOMAIN_NAME + '/DefaultRealm/myrealm/RoleMappers/XACMLRoleMapper'
        ]:
            try:
                cd(rt_path)
                cmo.setRoleExpression('Admin', None, NOVA_EXPR_COMPLETA)
                print('Configurado via domainRuntime(): ' + rt_path)
                sucesso = True
                break
            except Exception as e_path:
                print('Path ' + rt_path + ' falhou: ' + str(e_path.__class__.__name__))
    except Exception as e2:
        print('domainRuntime() falhou: ' + str(e2.__class__.__name__))
...
if sucesso:
    ...
    print('')
    print('WLST nao conseguiu configurar o XACMLRoleMapper nesta versao do WLS.')
    print('Configure manualmente via WRC:')
    print('  Security > Realms > myrealm > Role Mappers > XACMLRoleMapper > Realm Roles > Admin > Edit Role')
    print('  Expressao a colar:')
    print('  ' + NOVA_EXPR_COMPLETA)
PYEOF
