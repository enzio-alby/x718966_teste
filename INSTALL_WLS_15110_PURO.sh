...
# Em dominio novo nao ha policy previa — getRoleExpression lanca NotFoundException.
expr_str = ''
try:
...
print('Nova expressao calculada: ' + nova_expr)

# Abordagem 1: setRoleExpression direto (funciona em WLS 12.2.1.4)
sucesso = False
try:
    cmo.setRoleExpression('Admin', None, nova_expr)
    print('Configurado via modo direto.')
    sucesso = True
except Exception as e1:
    print('Modo direto falhou (' + str(e1.__class__.__name__) + '). Tentando edit mode...')

# Abordagem 2: via edit/startEdit (WLS 14.1.2+ pode exigir change manager)
if not sucesso:
    try:
        edit()
        startEdit()
        ...
        save()
        activate()
        print('Configurado via edit mode.')
        sucesso = True
    except Exception as e2:
        print('Edit mode falhou (' + str(e2.__class__.__name__) + '): ' + str(e2))
        try:
            cancelEdit('y')
        except:
            pass
...
if sucesso:
    ...
    print('Grupos: ' + str(grupos_add))
else:
    print('')
    print('XACML nao configurado via WLST. Configure manualmente via WRC:')
    print('  Security > Realms > myrealm > Role Mappers > XACMLRoleMapper > Realm Roles > Admin > Edit Role')
    print('  Expressao a inserir:')
    print('  ' + nova_expr)
    raise Exception('XACML requer configuracao manual')
PYEOF
