...
if [ "$TIPO" == "odi" ]; then
  ...
  ${SCHEMA_PREFIX}_OPSS  ${SCHEMA_PREFIX}_WLS  ${SCHEMA_PREFIX}_ODI_REPO"
    ...
    COMPONENTS="-component STB -component MDS -component IAU -component IAU_APPEND -component IAU_VIEWER -component OPSS -component WLS -component WCC -component IPM -component CAPTURE"
    SCHEMAS="STB MDS IAU IAU_APPEND IAU_VIEWER OPSS WLS WCC IPM CAPTURE"
    SCHEMAS_DISPLAY="${SCHEMA_PREFIX}_STB  ${SCHEMA_PREFIX}_MDS  ${SCHEMA_PREFIX}_IAU
  ${SCHEMA_PREFIX}_IAU_APPEND  ${SCHEMA_PREFIX}_IAU_VIEWER
  ${SCHEMA_PREFIX}_OPSS  ${SCHEMA_PREFIX}_WLS
  ${SCHEMA_PREFIX}_WCC  ${SCHEMA_PREFIX}_IPM  ${SCHEMA_PREFIX}_CAPTURE"
else
...
if [ "$TIPO" == "wcc" ]; then
    log_info "Proximo passo: re-executar o RCU via menu [4] do INSTALL_WCC_12214.sh"
elif [ "$TIPO" == "odi" ]; then
