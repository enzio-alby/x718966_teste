...
# ── Clusters (1 por servidor em QA; multi-node em HML/PRD) ──
# Padrao Oracle WCC: cada MS em seu cluster, DSes targetados ao cluster
print('Criando clusters...')
cluster_map = [
    ('wcc_cluster',   'UCM_server1'),
    ('ibr_cluster',   'IBR_server1'),
    ('ipm_cluster',   'IPM_server1'),
    ('cpt_cluster',   'capture_server1'),
    ('wccui_cluster', 'WCCADF_server1'),
]
cd('/')
for cluster_name, server_name in cluster_map:
    try:
        create(cluster_name, 'Cluster')
        cd('/Clusters/' + cluster_name)
        cmo.setClusterMessagingMode('unicast')
        print('  [OK] Cluster: ' + cluster_name)
    except Exception, e:
        print('  Cluster ja existe: ' + cluster_name)
    cd('/')
    try:
        cd('/Servers/' + server_name)
        set('Cluster', cluster_name)
        print('    ' + server_name + ' -> ' + cluster_name)
    except Exception, e:
        print('    Erro assign cluster ' + server_name + ': ' + str(e)[:60])
    cd('/')
...
# ── Datasources de produto criados pelo template (CSDS, IPMDS) ──
# capture-ds / capture-mds-ds / mds-WCCUIMDSREPO NAO vem de template — criados abaixo
print('Configurando datasources de template (CSDS, IPMDS)...')
ds_produto = [
...
# ── Datasources Capture e WCCADF MDS (criacao manual — sem template dedicado) ──
print('Criando datasources Capture e WCCADF MDS...')

def criar_jdbc_ds(ds_name, jndi_name, schema_user, target):
    try:
        cd('/')
        try:
            create(ds_name, 'JDBCSystemResource')
        except Exception, ec:
            print('  DS ja existe, reconfigurando: ' + ds_name)
        base = '/JDBCSystemResource/' + ds_name + '/JdbcResource/' + ds_name
        # Os filhos do JdbcResource NAO sao criados automaticamente em readDomain offline
        # e precisam ser criados explicitamente antes de navegar neles
        cd(base)
        for child in ['JDBCDataSourceParams', 'JDBCConnectionPoolParams', 'JDBCDriverParams']:
            try:
                create('NO_NAME_0', child)
            except Exception:
                pass
        # Properties fica dentro de JDBCDriverParams/NO_NAME_0
        cd(base + '/JDBCDriverParams/NO_NAME_0')
        try:
            create('NO_NAME_0', 'Properties')
        except Exception:
            pass
        # JDBCDataSourceParams — JNDI name
        cd(base + '/JDBCDataSourceParams/NO_NAME_0')
        cmo.addJNDIName(jndi_name)
        # JDBCConnectionPoolParams
        cd(base + '/JDBCConnectionPoolParams/NO_NAME_0')
        ...
        # Target no cluster
        cd('/')
        assign('JDBCSystemResource', ds_name, 'Target', target)
        print('  [OK] DS: ' + ds_name + ' (' + jndi_name + ') -> ' + target)
    except Exception, e:
        print('  Erro DS ' + ds_name + ': ' + str(e)[:100])

criar_jdbc_ds('capture-ds',       'jdbc/capture-ds',       SCHEMA_PFX + '_CAPTURE', 'cpt_cluster')
criar_jdbc_ds('capture-mds-ds',   'jdbc/capture-mds-ds',   SCHEMA_PFX + '_MDS',     'cpt_cluster')
criar_jdbc_ds('mds-WCCUIMDSREPO', 'jdbc/mds/WCCUIMDSREPO', SCHEMA_PFX + '_MDS',     'wccui_cluster')
cd('/')

# Re-targetar CSDS e IPMDS para clusters (padrao HML — sem remover target de servidor)
print('Re-targetando CSDS/IPMDS para clusters...')
for ds_name, cluster in [('CSDS', 'wcc_cluster'), ('IPMDS', 'ipm_cluster')]:
    try:
        assign('JDBCSystemResource', ds_name, 'Target', cluster)
        print('  Re-target: ' + ds_name + ' -> ' + cluster)
    except Exception, e:
        print('  Re-target ignorado ' + ds_name + ': ' + str(e)[:60])
cd('/')
...
PYEOF
    ...
    local wlst_tmp="${MIDIAS}/rcu_tmp"
    mkdir -p "$wlst_tmp"
    chown ${ORACLE_USER}:${ORACLE_GROUP} "$wlst_tmp"

    log_info "Executando WLST para criar domain..."
    sudo -u oracle bash -c "
        export JAVA_HOME=${JAVA_HOME}
        export ORACLE_HOME=${ORACLE_HOME}
        export _JAVA_OPTIONS='-Djava.io.tmpdir=${wlst_tmp}'
        cd '${DOMAIN_HOME}'
        '${wlst}' '${domain_py}'
    " 2>&1 | tee -a "$LOGFILE"
...
NMEOF
    ...
    # Criar tmpdir e setUserOverrides.sh — oracle sem acesso ao /tmp neste servidor
    local srv_tmp="${DOMAIN_HOME}/tmp"
    sudo -u oracle bash -c "
        mkdir -p '${srv_tmp}'
        printf 'export _JAVA_OPTIONS=\"-Djava.io.tmpdir=${srv_tmp}\"\n' \
            > '${DOMAIN_HOME}/bin/setUserOverrides.sh'
        chmod 755 '${DOMAIN_HOME}/bin/setUserOverrides.sh'
    "
    log_ok "setUserOverrides.sh configurado com tmpdir: ${srv_tmp}"
