...
aplicar_patches() {
    ...
    local opatch_src
    opatch_src=$(find opatch_update -maxdepth 2 -name "OPatch" -type d | head -1)
    if [[ -z "$opatch_src" ]]; then log_erro "Diretório OPatch não encontrado após extração."; exit 1; fi
    sudo -u oracle cp -r "$opatch_src" "${ORACLE_HOME}/"
