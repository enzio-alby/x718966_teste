...
coletar_infos() {
    ...
    [[ -z "$DB_HOST" ]] && { log_erro "Hostname do banco obrigatório."; exit 1; }

    read -rp "  Porta do listener [${DB_PORT}]: " inp
    DB_PORT="${inp:-$DB_PORT}"

    read -rp "  Service name do PDB (ex: WCCQASRV): " DB_PDB
    [[ -z "$DB_PDB" ]] && { log_erro "PDB obrigatório."; exit 1; }
    ...
    [[ -z "$WLS_ADMIN_PASS" ]] && { log_erro "Senha admin obrigatória."; exit 1; }

    read -rsp "  Senha do usuário SYS (DBA): " DB_SYS_PASS; echo ""
    [[ -z "$DB_SYS_PASS" ]] && { log_erro "Senha SYS obrigatória."; exit 1; }

    read -rsp "  Senha padrão para os schemas criados: " DB_SCHEMA_PASS; echo ""
    [[ -z "$DB_SCHEMA_PASS" ]] && { log_erro "Senha dos schemas obrigatória."; exit 1; }
...
instalar_binarios() {
    ...
    [[ -z "$java_bin" ]] && { log_erro "Java não encontrado. Instale o Java primeiro."; exit 1; }
...
aplicar_patches() {
    ...
    # ── Baixar tudo via wget mirror (mesma estrutura do COMPOSTO) ──
    ...
    cd "${BASE_WCC}/opatch"
    sudo -u oracle unzip -q "$OPATCH_ZIP" -d opatch_update
    sudo -u oracle cp -r opatch_update/OPatch "${ORACLE_HOME}/"
...
executar_rcu() {
    ...
    log_info "Schemas a criar: STB, MDS, IAU, IAU_APPEND, IAU_VIEWER, OPSS, WLS, WCC, IPM, CAPTURE"
    ...
    [[ "$conf_rcu" != "s" ]] && { log_info "RCU cancelado pelo usuário."; return 0; }
    ...
    local rcu_tmpdir="${ORACLE_HOME}/rcu_tmp"
    mkdir -p "$rcu_tmpdir"
    ...
    [ $rcu_exit -ne 0 ] && {
        log_erro "RCU falhou com código $rcu_exit. Verifique: /tmp/RCU*/logs/rcu.log"
        return 1
    }
...
# Template: Oracle WebCenter Content (Complete Install)
# =============================================================================
...
def createDomain():
    print('Criando domain WCC: ' + DOMAIN_NAME)

    selectTemplate('Basic WebLogic Server Domain')
    selectTemplate('Oracle WebCenter Content')
    loadTemplates()
    ...
    print('AdminServer: porta ' + str(ADMIN_PORT))

    # ── Managed Servers ──
    # Mapa: (nome-no-template, nome-final, porta)
    ms_map = [
        ('UCM_server1',     'UCM_server1',     PORT_UCM),
        ('IBR_server1',     'IBR_server1',     PORT_IBR),
        ('IPM_server1',     'IPM_server1',     PORT_IPM),
        ('capture_server1', 'capture_server1', PORT_CAP),
        ('WCCADF_server1',  'WCCADF_server1',  PORT_ADF),
    ]

    for tpl_nome, ms_nome, ms_port in ms_map:
        try:
            cd('/Servers/' + tpl_nome)
            ...
            print('MS nao encontrado no template: ' + tpl_nome + ' | ' + str(e)[:60])
            try:
                cd('/')
                ...
                print('ERRO ao criar MS ' + ms_nome + ': ' + str(e2)[:80])
    ...
    try:
        ...
        print('Erro ao configurar NM da machine: ' + str(e)[:60])

    # Atribuir todos os servidores a machine
    ...
    for srv in servidores:
        try:
            ...
            print('Assign ignorado para ' + srv + ': ' + str(e)[:60])

    # ── DataSources JDBC ──
    print('Configurando DataSources JDBC...')
    jdbc_url = 'jdbc:oracle:thin:@//' + DB_HOST + ':' + str(DB_PORT) + '/' + DB_PDB

    # (nome-datasource, schema-suffix)
    ds_list = [
        ('LocalSvcTblDataSource', SCHEMA_PFX + '_STB'),
        ('mds-owsm',              SCHEMA_PFX + '_MDS'),
        ('mds-wcc',               SCHEMA_PFX + '_MDS'),
        ('opss-data-source',      SCHEMA_PFX + '_OPSS'),
        ('opss-audit-DBDS',       SCHEMA_PFX + '_IAU_APPEND'),
        ('opss-audit-viewDS',     SCHEMA_PFX + '_IAU_VIEWER'),
        ('wccds',                 SCHEMA_PFX + '_WCC'),
        ('IPMDS',                 SCHEMA_PFX + '_IPM'),
        ('CaptureDataSource',     SCHEMA_PFX + '_CAPTURE'),
    ]

    for ds_name, schema in ds_list:
        try:
            ...
            print('  DS ignorado (normal no template): ' + ds_name + ' | ' + str(e)[:80])
...
iniciar_servidores() {
    ...
    # boot.properties
    local boot_dir="${DOMAIN_HOME}/servers/AdminServer/security"
    sudo -u oracle bash -c "
        mkdir -p '${boot_dir}'
        printf 'username=${WLS_ADMIN_USER}\npassword=${WLS_ADMIN_PASS}\n' > '${boot_dir}/boot.properties'
        chmod 600 '${boot_dir}/boot.properties'
    "
...
NMEOF
    ...
    sudo -u oracle bash -c "
        ...
        nohup '${DOMAIN_HOME}/bin/startNodeManager.sh' \
            > /u02/midias/logs/nm_wcc.log 2>&1 &
        disown
    "
    sleep 20
    ...
    sudo -u oracle bash -c "
        ...
        nohup '${DOMAIN_HOME}/bin/startWebLogic.sh' \
            > /u02/midias/logs/admin_wcc.log 2>&1 &
        disown
    "
    ...
    if sudo -u oracle bash -c "timeout 5 bash -c 'echo >/dev/tcp/${HOST_NAME}/${ADMIN_PORT}'" 2>/dev/null; then
        ...
        log_warn "Admin Server não respondeu ainda. Acompanhe: tail -f /u02/midias/logs/admin_wcc.log"
    fi
    ...
    iniciar_ms() {
        ...
        sudo -u oracle bash -c "
            ...
            nohup '${DOMAIN_HOME}/bin/startManagedWebLogic.sh' \
                ...
                > /u02/midias/logs/${nome}.log 2>&1 &
            disown
        "
        log_ok "${nome} iniciado. Log: /u02/midias/logs/${nome}.log"
    }
