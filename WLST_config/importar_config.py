...
#                        Deixar vazio para manter URLs originais do JSON.
...
def substituir_banco_url(url, novo_banco):
    """
    Substitui o host:porta/servico (ou host:porta:SID) na URL JDBC.
    Suporta os formatos Oracle mais comuns:
    ...
    if not url or not novo_banco:
        return url
    # Formato @//host:port/service (URL style)
...
# Atualiza args JVM de servidores ja existentes — nao cria servidores novos.
# ─────────────────────────────────────────────────────────────────────────────
...
def importar_jdbc(datasources, dry_run, db_override):
    ...
    for ds in datasources:
        ...
        try:
            ...
            cmo.setPassword('')  # senha redefinida manualmente no console
...
def main():
    ...
    db_override     = os.environ.get('WLS_DB_OVERRIDE', '').strip()
